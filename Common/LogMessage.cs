...
namespace Common
{
    public class LogMessage
    {
        ...
        {
            StringBuilder msg = new StringBuilder("------执行 " + MessageBody + " 生成时间：" + DateTime.Now + "---\r\n");
            msg.Append(ex + "\r\n\r\n");
            ...
            {
                Common.St.SaveToFile(msg, System.AppDomain.CurrentDomain.BaseDirectory.TrimEnd('\\') + "\\ErrLog\\Log_" + fileName + ".log", true);
            }
            else
            {
                string LogFilePath = FilePath + "Log_" + fileName;
